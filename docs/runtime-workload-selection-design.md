...
After step 4, **every** non-default scenario uses `spawn_with_workload`; `spawn_with_features` survives only as the internal `&[]` default-build call. The build-dedup (point below) is now fully unblocked.
