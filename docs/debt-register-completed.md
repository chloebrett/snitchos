# Technical / architectural debt — closed entries

The completed half of the [debt register](debt-register.md). An entry moves here
when it is done rather than being deleted, because the write-up is usually worth
more than the item was — several of these record a *misdiagnosis* and how it was
caught, which is exactly what gets lost when a backlog line is ticked off.

Numbers are never reused. `#N` names the same entry here as it did in the live
register, so an inbound reference from a post, a plan or a source comment still
resolves after the move.

Ordered as they were closed, not by number.
