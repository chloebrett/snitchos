...
- **MemoryRegion** — a chunk of physical memory + the right to map it into an address space. The canonical microkernel cap (seL4's Frame/Untyped). **Deliberately *not* v0.7b** — it has no consumer until something grows or *shares* memory, and a cap with no consumer is machinery for its own sake. Its real reason to exist is shared memory between two processes, so it lands with **v0.8 IPC**. Distinct from an allocator: a `malloc` subdivides bytes *within* memory you already hold (retail); a `MemoryRegion` cap is how you *acquired* that memory and the right to place it in an address space (wholesale). See [v0.7b plan](../plans/legacy/v0.7b-capabilities.md) for why it's excluded.
...
**v0.7b status — the handle is handed over (register-passed).** The process *holds* a real, validated capability **and is told its handle**: `enter` sets `a0` to the granted handle before `sret`, `crt0` forwards `a0` into `rust_main(startup: Startup)`, and the program calls `startup.telemetry()`. The old hardcoded `TELEMETRY_SINK_HANDLE` is gone — grant the cap anywhere and the program uses what it was told (teeth-checked: a wrong `a0` makes the telemetry vanish). One register carries one handle, which is exactly v0.7b's single-cap case.

**The mechanism (v0.8): a memory `BootInfo` for *many* caps.** When caps multiply, `a0` becomes a pointer to an in-memory startup structure listing many handles + what each is — seL4's `seL4_BootInfo`, structurally **Linux's `auxv`** (kernel writes startup facts at entry, the runtime captures them, the kernel forgets them; env vars are the same shape one layer up). The program-facing `Startup` type (`repr(transparent)` over `usize`) stays put across that change, so the migration is contained to `enter` + `crt0`. Transfer-over-IPC (`Endpoint`) is how caps flow *after* startup; `BootInfo` is how the *initial* set arrives. See [the v0.7b plan](../plans/legacy/v0.7b-capabilities.md).
