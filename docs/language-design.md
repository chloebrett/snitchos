...
**Operators as functions** — a bare operator in argument position _is_ its binary function: `fold(0, +)`, `reduce(max)`, `sortBy(<)`. Haskell-style sections (`(* 2)`) are deliberately _omitted_ — `$` already covers partial application, and one way is enough. _Implemented (recognised when an operator is immediately followed by `,`/`)`) as a fixed **binary** desugaring `op` ⇒ `(lhs, rhs) -> lhs op rhs`._ **Limitation:** because it's always binary, an operator with a unary meaning can't be used that way — `-` desugars to subtraction, never negation; for a unary use write the explicit lambda (`map($, x -> 0 - x)`). Other arities likewise need an explicit lambda. (`parser.rs::operator_lambda`.)
...
- **Mutability is opt-in:** fields are immutable unless marked `mut` (`prod Counter(mut n: Int)`). **Correction (2026-07-26, against the shipped interpreter — this bullet previously claimed the opposite): this is value semantics with call-site write-back, not Java/Kotlin shared-reference mutation.** Assigning `@field` (directly, or via a `mut` method) rebuilds the record and writes the result back only to *the place the method was called on* — a `Var`, `@`, or a field path (`interp.rs::eval_method_call`/`assign_place`; proven by its own tests, `field_assignment_does_not_alias_a_copy` and `a_mut_method_does_not_alias_a_copy`). Two bindings that happen to hold "the same" record at some point are not aliases of one heap cell — mutating one never touches the other, because each is its own copy from the moment they diverged. The practical bite: an object pulled out of a `List` (via `find`, indexing, pattern-matching, …) is a copy: mutating it does nothing to the list, and a caller must explicitly write the updated value back in (`examples/stitch/bank.st`'s `Ledger`/`replaceAccount` is the worked example). Calling a `mut` method requires a `mut` receiver binding, enforced today at *runtime* rather than statically — a plain `let` binding faults on the call ("cannot assign to immutable `x` — declare it with `let mut`").
- **Equality:** structural for **all** `prod`/`sum`, mutable fields or not — two `Point(1, 2)` are equal (Kotlin `data class` semantics, which compare every field whether `val` or `var`). The mutable-hash-key footgun (a key whose hash changes out from under the table when the value is mutated) is designed out *not* by faking identity equality, but by separating equality from **key-eligibility**: only immutable types would satisfy a `Key`/`Hashable` contract, so a value with any `mut` field could not be a `Map`/`Set` key. (Python forbids it at runtime — lists are unhashable; static typing would turn that into a type error.) Structural equality on a mutable value therefore means "equal as of now." `===` is always identity.
  - **Correction (2026-08-25): this bullet previously said such a key is "a compile error". It is not — the rule is designed, not built.** There is no `Key`/`Hashable` contract in `stitch/src/`, and no check anywhere rejects a `mut`-field key. The design intent above is worth keeping; the claim that it is *enforced* was false and had been read as fact. What actually happens today is milder than the footgun the rule was written against: `Value::Map` is an association vector compared structurally, so a mutated key does not corrupt a hash bucket — it simply stops comparing equal to the key you inserted, and lookups silently miss. Enforcing this needs checker work and is unscheduled.
...
- **`Map` is a dictionary you can build** (`Map.insert`/`remove`/`update`/`fromList`/`get`/`has`/`keys`/`values`/`entries`/`mapValues`, plus the `m[k]` index form which returns a `Maybe`). Three properties are **contract**, not implementation detail:
  - **Insertion-ordered.** `Map.keys` and every iteration yield entries in the order they were first inserted; re-inserting an existing key keeps its original position and takes the new value. That is exactly what the `[k: v, …]` literal already does with a duplicate key, and matching it is what makes a *built* map and a *written* one interchangeable. Note equality is **unordered** — two maps with the same entries in different orders compare equal — so a test that means to pin order must say so through `Map.keys`, not through `==`. (That trap cost real coverage twice while the module was being built.)
  - **Persistent.** Every operation returns a new `Map`; the receiver is untouched, like `List.set`/`List.insert`.
  - **In the combinator vocabulary.** `fold`/`map`/`filter` accept a `Map`, iterating `(key, value)` tuples — which is what gives `count`/`any`/`all`/`find` and the rest of the fold-derived prelude their `Map` support for free, with no `Map.`-prefixed duplicates. `map` returns a **`List`** (projecting an entry to an arbitrary value loses the key shape) while `filter` returns a **`Map`** (a predicate selects entries, it doesn't transform them). The asymmetry is deliberate, and matches Kotlin.

  `Map.update(m, k, default, f)` inserts `default` **raw** when the key is absent and `f(existing)` when present — as in Rust's `entry(k).or_insert(d)`, Haskell's `insertWith`, Kotlin's `merge` and Elixir's `Map.update/4`. So a tally is one expression: `fold(words, [:], (acc, w) -> Map.update(acc, w, 1, $ + 1))`. The alternative reading (apply `f` to the default) is the special case reached by passing `f(default)`, and cannot express a first-seen value outside `f`'s image.

  Representation today is an association vector, so lookup is **O(n)** — the API makes the shape idiomatic while the cost stays linear. Swapping in a hash or tree is interior work for the VM stage, bound by the ordering contract above. Rationale and history: [../plans/legacy/stitch-map-you-can-build.md](../plans/legacy/stitch-map-you-can-build.md).
- **`Set<T>` does not exist.** It is specified in [../plans/lang/01-grammar-and-precedence.md](../plans/lang/01-grammar-and-precedence.md) ("eager; no literal, `[1,2,3] |> toSet`") and was never built — no `Value::Set`, no `toSet`. Thirty example programs produced no need for set algebra (`spellcheck.st` wanted membership and `List` + `contains` covered it), so the gap is recorded rather than filled. Do not write code against `Set` expecting it to be there.
...
- `mut` methods may write `@`; calling one **requires a `mut` receiver binding** (Rust's `&mut self` discipline, no lifetimes) — checked at runtime today, not yet by the checker. The final `@` is written back only to the exact expression the method was called on when the call returns; see "Mutability is opt-in" above for what that does and doesn't alias.
...
- **Syntax & type system.** Now substantially worked out — see [Surface syntax](#surface-syntax) and [Type system](#type-system) above. **Modules & visibility (iteration 1) are now built** — file-per-module, `ext` to export (private by default, the twin of `mut`/`uses`), `use M` / `use M.{a, b}` to import, namespaced built-in stdlib modules (`Seq`/`Str`), and the `contract`-coherence/orphan rule enforced at registration. Design + build notes: [plans/lang/06-modules-and-visibility.md](../plans/lang/06-modules-and-visibility.md). Remaining grammar gaps: generics beyond `List<T>` (bounds, variance), opaque types (field/constructor visibility), and the precise `match` pattern grammar.
- **The self-type `@` (deferred to the type system).** `@` is accepted in type position today (`unwrap() -> @`, `map(f) -> @` in the `Try`/`Functor` contracts) and parses to `Type::SelfType`, but — like every v0 type annotation — it is **parsed and ignored**, with no checking. When the type system lands it needs *meaning* (`@` = the receiver's own type, so `Maybe`'s `map` provably returns a `Maybe`; ties into conformance checking + the monomorphization path) and *gating* (it should be a parse/type error outside an `on`/`contract` method signature — a top-level `foo() -> @` has no receiver). Both are out of scope until the checker exists; the parse-only acceptance just lets the contract signatures read honestly now.
