...
A living backlog of elegance / architectural debt, opened during the v0.10 work
and added to since. Each item is independently actionable. Ordered by leverage,
not urgency.

This file holds what is **open**. When an item is done, move its write-up to
[debt-register-completed.md](debt-register-completed.md) rather than deleting it
— keeping its number — and add new items here. This is a register, not a plan:
see `plans/` for active implementation tracks.

Numbering is a single sequence across both files, so the next new entry takes the
number after the highest one in either.
