...
2. **Extend the existing `#[entry]` macro** (`user/macros`) to
   ...
   bytes:
   ```rust
   #[link_section = ".note.snitch.iface"]
   #[used]
   static IFACE: [u8; N] = /* hitched manifest */;
   ```
...
3. **The seed step extracts it.** `user/fs/build.rs` (the declared-executable
   ELF-injection) reads the `.note.snitch.iface` section out of the linked ELF with
   the `object` crate and writes those bytes to `user.iface` in the `SEED`.
...
- **Linker `--gc-sections` can drop the section.** `#[used]` stops *Rust's*
  dead-code pass, but the linker may still GC it — likely need `KEEP(.note.snitch.iface)`
  in the userspace linker script (or `#[used(linker)]`). The detail that bites first.
...
- Rust build path = **`#[derive(Schema)]` + extend `#[entry]` → `.note.snitch.iface`
