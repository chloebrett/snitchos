...
impl Manifest {
    ...
    pub fn render(&self) -> String {
        format!(
            "seed={}\nprogram_count={}\nprobe_digest={}\n",
            self.seed, self.program_count, self.probe_digest
        )
    }
...
/// Separates programs in a corpus file. ASCII Record Separator — outside
/// Stitch's lexical alphabet, so it cannot appear inside a program and the
/// split needs no escaping.
...
pub fn render_corpus(programs: &[String]) -> String {
    programs.join(&SEPARATOR.to_string())
}
