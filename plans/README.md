...
| [stitch-map-you-can-build.md](stitch-map-you-can-build.md) | A `Map` built from runtime data | **Further along than its own criteria say** — 10 natives shipped (`insert`/`fromList`/`entries`/`keys`/`values`/`get`/`has`/`remove`/`update`/`mapValues`). Left: combinator integration (`map`/`filter` still call `expect_list`) and the corpus migration — **0 of the 12 hand-rolled association lists retired**, which was the plan's whole point |
