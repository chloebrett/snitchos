...
| [stitch-map-you-can-build.md](stitch-map-you-can-build.md) | A `Map` built from runtime data | **Effectively DONE; header still says Active.** 10 natives shipped, `fold`/`map`/`filter` accept a `Map` (`a42679d`, 2026-08-06) which lit up `count`/`any`/`all`/`find`, and `logstats.st`/`inventory.st` are rewritten (`b8a0acc`). All 8 criteria still unticked. **Verify and archive** — 10 of the 12 workaround files are unconverted, which the findings doc treats as residue, not debt |
