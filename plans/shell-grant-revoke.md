...
2. **`grant <handle> <badge> <rights>`** — `MintBadged`. Platform method + `native_grant` + tests: granting from a `MINT` endpoint appends a new cap (visible in `hold`); granting from a non-MINT cap refuses. Rights parsing (`SEND`/`RECV`/… → bitmask) — reuse `snitchos_abi::rights`.
3. **The loop, on the metal** — an itest (or a manual `cargo xtask boot --workload stitch-fs`) driving `hold` → `grant` → `hold` → `revoke` → `hold`, asserting the `CapEvent::Transferred` then `CapEvent::Revoked` frames on the wire.
