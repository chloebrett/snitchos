...
use cram_gen::{
    Model, extract, prompt, run_once, run_once_capped, run_once_corrected, run_once_guarded,
};
...
/// A model whose chunk boundaries are given explicitly.
///
/// `Sequence` splits on newlines, which cannot express the failure this exists
/// for: the separator a rewind loses is *inside* a line, between two tokens.
struct Chunks {
    replies: std::cell::RefCell<Vec<Vec<String>>>,
    prefills: std::cell::RefCell<Vec<String>>,
}
...
impl Chunks {
    fn new(replies: &[&[&str]]) -> Self {
        Self {
            replies: std::cell::RefCell::new(
                replies.iter().rev().map(|r| r.iter().map(|c| (*c).to_string()).collect()).collect(),
            ),
            prefills: std::cell::RefCell::new(Vec::new()),
        }
    }
...
/// Finding 2 of `notes/batch9-findings.md`, as a test. Resuming from a prefill
/// loses one separator at the boundary — the chat template strips it, or the
/// model will not open a continuation with a leading space — so a rewind whose
/// discarded text began with whitespace comes back without it and the two
/// tokens glue: `ext` + ` freeSpace` became `extfreeSpace` in 97% of batch9's
/// files, and that glue *is* the parse-error population.
#[test]
fn a_rewind_puts_back_the_separator_the_resume_dropped() {
    let model = Chunks::new(&[
        // Dies on the type alias. The fatal chunk begins with a space, and the
        // chunk before it is a bare `ext`.
        &["```stitch\n", "ext f() -> Int = 1\n", "ext", " Time = Int\n"],
        // The resume re-emits the identifier without its separator.
        &["double(n: Int) -> Int = n * 2\n```"],
    ]);

    let run = run_once_corrected(&model, "anything", 2, &mut |_| {}, &mut |_| {})
        .expect("fake succeeds");

    assert!(!run.program.contains("extdouble"), "tokens glued: {:?}", run.program);
    assert!(run.program.contains("ext double(n: Int)"), "got {:?}", run.program);
    // The correction still records what the model actually said. It is evidence
    // about the model, not about the splice.
    assert_eq!(run.corrections[0].replacement, "double(n: Int) -> Int = n * 2\n```");
}

/// Only when the separator is genuinely missing. A replacement that brings its
/// own whitespace must not be given a second copy — that would turn every
/// indentation rewind into a creeping indent.
#[test]
fn a_replacement_that_kept_its_separator_is_left_alone() {
    let model = Chunks::new(&[
        &["```stitch\n", "ext f() -> Int = 1\n", "ext", " Time = Int\n"],
        &[" double(n: Int) -> Int = n * 2\n```"],
    ]);
    ...
    assert!(run.program.contains("ext double(n: Int)"), "got {:?}", run.program);
    assert!(!run.program.contains("ext  double"), "double separator: {:?}", run.program);
}

/// A cap on the *accumulated* program, which nothing bounded before.
///
/// `max_tokens` bounds one completion; `run_once_corrected` issues many, so a
/// candidate that keeps being rewound can grow without limit — batch9's worst
/// reached 197 KB. And length is the yield: 25% of batch9's files passed 12 KB
/// and **80% of those died at parse**, against a 45% base rate. Stopping there
/// costs 5% of the corpus (the 48 that would have survived) and reclaims ~14%
/// of the wall clock, which is the constraint on how big a corpus can get.
#[test]
fn generation_stops_once_the_program_outgrows_the_cap() {
    let long = format!("ext a() -> Int = 1\n{}\n", "// narration\n".repeat(400));
    let model = Fake(format!("```stitch\n{long}```"));

    let run = run_once_capped(&model, "anything", 2, 500, &mut |_| {}, &mut |_| {})
        .expect("fake succeeds");

    assert!(run.overlong, "the cap should have fired");
    assert!(run.program.len() < 2000, "generation should have stopped near the cap");
}

/// The cap must not touch a program that stays inside it, and `overlong` says
/// which happened — a truncated candidate and a naturally-short one both reach
/// the gate, and only one of them is evidence about Stitch.
#[test]
fn a_program_inside_the_cap_is_untouched() {
    let run = run_once_capped(&Fake(format!("```stitch\n{GOOD}```")), "anything", 2, 100_000, &mut |_| {}, &mut |_| {})
        .expect("fake succeeds");
...
fn a_cap_of_zero_never_fires() {
    ...
    let run = run_once_capped(&Fake(format!("```stitch\n{long}```")), "anything", 2, 0, &mut |_| {}, &mut |_| {})
        .expect("fake succeeds");

    assert!(!run.overlong);
    assert!(run.program.len() > 4000, "nothing should have been cut");
}
...
fn a_candidate_record_serialises_the_whole_funnel_state() {
    ...
    for expected in [
        ...
        "\"size\":\"small\"",
        "\"shape\":\"module\"",
        // Whether the guard gave up is the difference between "the model wrote
        // this" and "the model wrote this after correction ran out", and it was
        // computed and thrown away for all 973 of batch9.
        "\"abandoned\":true",
    ] {
...
/// Prose before the fence is dropped, so a model that thinks out loud first
/// does not pay for it in the corpus.
///
/// Kept as documentation of a *failed* experiment. Pilot 2 added a prompt line
/// inviting the model to plan here instead of in comments; 9 of 10 responses
/// opened straight into the fence, and the comment fraction the change existed
/// to reduce went from 60.0% to 72.6%. The prompt was reverted — the extraction
/// behaviour it relied on is real, the behaviour change it assumed was not.
#[test]
fn a_plan_before_the_fence_never_reaches_the_program() {
    let planned = format!(
        "I need a record type and two functions. Sorting first, then the fold.\n\n```stitch\n{GOOD}```"
    );

    let run = run_once(&Fake(planned), "anything", &mut |_| {}).expect("fake succeeds");

    assert!(!run.program.contains("I need a record type"), "plan leaked: {:?}", run.program);
    assert_eq!(run.outcome.stage(), "ok");
