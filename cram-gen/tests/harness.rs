...
/// The batch10 pilot's finding: 60% of every generated byte was comment, up from
/// batch9's 47.8%, because `enable_thinking: false` leaves the model nowhere to
/// plan except the file — and the three longest candidates, all of them
/// narrated, were the three that died at parse.
///
/// Two halves, and the second is what makes the first obeyable. A rule a model
/// cannot satisfy gets ignored — the existing "keep comments to one or two
/// lines" already was — so the prompt has to offer somewhere else to put the
/// reasoning before it can forbid the file. Prose before the fence is free:
/// `extract` drops everything outside the block, so it never reaches the corpus.
#[test]
fn the_system_prompt_gives_reasoning_somewhere_to_go_other_than_the_file() {
    let system = prompt::system();

    assert!(system.contains("before the block"), "no escape hatch for planning: {system}");
    assert!(system.contains("discarded"), "the hatch must say the prose is thrown away");
    assert!(
        system.to_lowercase().contains("do not plan in comments")
            || system.to_lowercase().contains("no planning comments"),
        "planning comments are not forbidden: {system}"
    );
}

/// And the hatch must not cost what it saves: prose before the fence is dropped,
/// so a plan does not become part of the program.
#[test]
