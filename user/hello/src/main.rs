...
// A plain `fn main()` — the runtime entry shim is generated by the macro,
// which stashes the startup capabilities so `telemetry()` / `tracer()` can
// reach them, calls us, then `exit()`s. No `rust_main`, no manual entry.
#[snitchos_user::main]
fn main() {
