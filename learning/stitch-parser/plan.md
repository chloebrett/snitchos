...
- [x] **S7 — Bridge to dispatch → IMPLEMENTED.** Learner built basic `on X` method dispatch end to end: registry (`register_items` → `HashMap<String, Vec<Method>>`), Env bridge (`methods` OnceCell + `lookup_method` + `globals_only`), the `Call{Field}` interception arm, and the `SelfRef`/`@` arm. All red tests green (292+11+3). Track destination reached. **Deferred:** `on X : C` contract default-method fallback; method modifiers at eval.

## Progress
- Session log: `session-log.md`. Cheat sheet: `cheat-sheet.md` (built as we go).
- Status: **COMPLETE. S1–S7 done. Learner implemented basic `on X` contract dispatch end to end (the track's destination). Remaining language work (contract conformance + default methods, method modifiers) is regular dev, not learning track.**
