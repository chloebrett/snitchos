...
Up next: **v0.5 (threading + round-robin scheduler)**. Allocator A/B (`talc` vs `linked_list_allocator`) is a fast-follow gated on richer heap instrumentation (fragmentation, size buckets); fragmentation introspection will likely involve forking LLA.
