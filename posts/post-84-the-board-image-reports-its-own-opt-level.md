# Post 84 — the board image reports its own optimisation level
...
- [post 80](post-80-checkpoint-vocab-pairing.md) was about a guard that passes while checking nothing. [post 82](post-82-snitchos-in-a-browser-tab.md) was about a diagnosis inherited rather than derived. this one is the same family again, and I did not notice until the third instance.
...
- I spent most of a session reasoning from **"11.8s for a six-token completion"**, which is in [post 74](post-74-trained-weights-answer-tab-on-target.md) and in the archived plan, and which was true when written.
...
- [post 79](post-79-correcting-a-wrong-diagnosis.md) is about transcription being the least-instrumented hop. this is the hop after that: a number that was correctly transcribed and has since stopped being true. the note now carries a warning where the figure used to be quoted.
...
- and the first draft's long prefix was already grammatically dead, so its row silently reported the cost of seventeen refusals rather than the cost of a long line. prefixes are now asserted viable before use. [post 73](post-73-cram-eval-the-baseline-nobody-measured.md)'s lesson — print the samples beside the metric — arriving in a new costume.
