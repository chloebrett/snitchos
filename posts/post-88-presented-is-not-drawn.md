# Post 88 — presented is not drawn
...
**2. snemu and QEMU disagreed about who gets a framebuffer.** QEMU keys off the scenario's `ramfb` *tag*; snemu keyed off the *workload string*. a scenario correctly tagged but on a differently-named workload got a framebuffer under QEMU and none under snemu — and because of bug 1, that looked like it worked. two sources of truth for one fact, which is the same shape as [post 83](post-83-the-file-disagreed-with-itself.md). patched, with the hazard written into the code; the real fix derives it from the catalog's tags.
...
and the honest limit, in the plan: if the font *table* were wrong, drawing and reading with the same wrong data cancels out. that is [post 87](post-87-agreement-was-not-evidence.md)'s failure mode, and the font has its own tests for exactly that reason.
