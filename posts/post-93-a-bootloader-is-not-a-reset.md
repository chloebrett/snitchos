# Post 93 — a bootloader is not a reset

- [post 92](post-92-every-write-succeeded.md) got telemetry across a real UART and ended with the board wedging a few instructions into its own boot. this session found out why: **the kernel inherits U-Boot's PLIC interrupt-enable bitmap**, then livelocks on the first interrupt it is handed for a device it has never heard of.
