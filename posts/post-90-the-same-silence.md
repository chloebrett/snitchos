...
- [post 87](post-87-agreement-was-not-evidence.md) was about artifacts confirming each other because they shared an ancestor. [post 88](post-88-presented-is-not-drawn.md) was about a proxy that cannot fail. [post 89](post-89-nothing-to-report.md) was about an observable that could not fire. this one is the same family seen from the other side: **two different failures that return the same value**, so no test can tell them apart — and therefore no test fails.
...
- **the strongest failure message names what it saw, not what it wanted.** `primed=true, xruns_total=false, frame=true` was [post 89](post-89-nothing-to-report.md)'s; this session's equivalent is exit `1` vs exit `2`, and the whole bridge is built so an unattended loop can tell them apart.
