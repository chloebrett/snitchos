...
- in-process also means the editor is a modal takeover: on the metal the read loop never ends (a serial line has no EOF), so `:stim` never returns to the prompt and Ctrl-C kills the whole shell. un-exitable on purpose, for now — `:q` and a real spawn both live on the same fast-follow list, along with the rest of the grammar I deferred.
