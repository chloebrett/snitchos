...
- **a token count is meaningless without naming its vocab**, and I confused myself with my own numbers for ten minutes. the identical 9 787 332 bytes of training text is **6 397 964** tokens under the babble-trained 1024 vocab and **2 933 322** under the batch9-trained 2048 one, at 1.53 vs 3.34 bytes/token. I reported the first, then the second, and the corpus looked like it had lost half its content. bytes are the vocab-independent anchor. this is exactly the trap [post 73](post-73-the-floor-was-in-the-wrong-place.md) names one level up — loss is not comparable across corpora — and it catches plain counts too.
