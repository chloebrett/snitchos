...
// Host-gate planning logic (workspace_members / unit_test_plan / riscv_only_plan
// / lints policy / run_unit_tests) moved to `crate::plan` — it implements the
// snemu-free `test`/`clippy`/`mutants` commands and must not live in this
// snemu-linked runner module. See plans/xtask-lean-test-binary.md (Step 1a).
