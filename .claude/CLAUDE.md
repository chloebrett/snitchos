...
- **`kernel-panic-emits-frame`** — `workload=panic-now` (a kernel task that just `panic!()`s). Asserts a `Log("kernel panic …")` reaches the wire: the panic handler emits telemetry, not just the emergency UART. The emit is panic-safe — no alloc, no intern (reuses `Frame::Log`), non-blocking bounded-retry `try_send_panic` — see [plans/panic-emits-telemetry.md](../plans/panic-emits-telemetry.md). `snemu-diff` conditions its benign-`kernel.heartbeat` pass on this frame (proof snemu reached the crash, not a fail-to-halt). `panic-now` also began as the minimal repro that isolated the guard-page family's snemu-vs-QEMU divergence to crash-vs-heartbeat *timing*, not MMU/scheduling.
