...
- **Release itests build the kernel at opt-3 but pin the embedded userspace to opt-1** (`kernel/build.rs`, nested build `--config profile.release.opt-level=1`). The pin was added to dodge a supposed "latent opt≥2 UB *class*" in the userspace crates. **There is no such class — root-caused 2026-07-29 (debt #16).** `memhog`'s 4 MiB reservation was dead code (`capacity()` does not keep an allocation alive) and LLVM deleted it at opt≥2, so the child committed no frames and the reclaim assertion failed; one `core::hint::black_box` fixed it. The suite is now **130/130 at opt-2 and opt-3**, and green on QEMU at opt-3 where a hang was documented. The pin survives on inertia only: removing it collapses `OptLevel::Mid` into `Max` and nothing was verified on the VF2, so it is a deliberate call, not a cleanup. **Lesson: "it spins at -O2" was inferred from an instret count; the profile said userspace was <1% of the run. Read the wire/profile before believing the summary.**
